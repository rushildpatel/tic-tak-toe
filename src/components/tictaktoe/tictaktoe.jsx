...
const TicTakToe = () => {
  ...
  const won = (winner) => {
    setLock(true);
    if (winner === "x") {
      titleRef.current.innerHTML = `Congratulations Winner <img src="${cross_icon}"></img>`;
    } else if (winner === "o") {
      titleRef.current.innerHTML = `Congratulations Winner <img src="${circle_icon}"></img>`;
    } else {
  ...
  const reset = () => {
    ...
    setCount((count) => 0);
    data = ["", "", "", "", "", "", "", "", ""];
    titleRef.current.innerHTML = `TIC TAK TOE in&#160;<span>React</span>`;
    box_arr.map((e) => {
      e.current.innerHTML = "";
    });
  };
  ...
  return (
    <div className="container">
      <h1 className="title" ref={titleRef}>
        TIC TAK TOE in&#160;<span>React</span>
      </h1>
